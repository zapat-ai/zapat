...
Verify Node.js is version 18 or higher. Verify git is version 2.20 or higher.
...
After all tools are verified, check if agent teams are enabled:

```bash
cat ~/.claude/settings.json 2>/dev/null | jq -r '.env.CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS // empty'
```

If not `"1"`, enable it automatically:
...
Create `config/project-context.txt` inside `$ZAPAT_HOME` combining the user's project description with auto-detected information:
...
Write `config/repos.conf` from the repos collected in Step 3:
```
...
cp $ZAPAT_HOME/agents/*.md ~/.claude/agents/
```
...
```bash
cd $ZAPAT_HOME && npm install
```

### Build dashboard
```bash
cd $ZAPAT_HOME/dashboard && npm install && npm run build
```
...
If yes, for each repo collected in Step 3:
```bash
REPO_PATH="/path/to/repo"  # from repos.conf
mkdir -p "$REPO_PATH/.github/ISSUE_TEMPLATE"
cp $ZAPAT_HOME/examples/issue-templates/*.yml "$REPO_PATH/.github/ISSUE_TEMPLATE/"

mkdir -p "$REPO_PATH/.github/workflows"
cp $ZAPAT_HOME/examples/github-actions/zapat-auto-label.yml "$REPO_PATH/.github/workflows/"
```
